...
- `kei sync --refresh-metadata` re-applies corrected metadata to already-downloaded assets. It clears stored metadata hashes so a one-time full re-enumeration refreshes the catalogue and rewrites embedded and sidecar metadata without re-downloading the media. Use it after upgrading to a build that fixes a metadata decode or capture bug. ([#673])
